<template>
	<div class="area-content">
		<div class="area-title">
			<router-link :to="{ path: '/project/collectInformation'}">
				<span> < 返回</span>
			</router-link>
		...
		<div>
			...
			<el-table :data="tables.slice((currentPage-1)*pageSize,currentPage*pageSize)" highlight-current-row style="width: 100%;">
				...
				<el-table-column prop="id" label="评估编号">
				</el-table-column>
				<el-table-column prop="houseOwner" label="房屋使用人" sortable>
				</el-table-column>
				<el-table-column prop="area" label="房屋坐落" sortable>
				</el-table-column>
				<el-table-column prop="estateAppraiser" label="评估员" sortable>
				</el-table-column>
				<el-table-column prop="technicalLeader" label="技术主管" sortable>
				</el-table-column>
				<el-table-column prop="projectLeader" label="项目主管" sortable>
				</el-table-column>
				<el-table-column prop="totalLandArea" label="土地面积" sortable>
				</el-table-column>
				<el-table-column prop="totalBuildArea" label="建筑面积" sortable>
				</el-table-column>
				<el-table-column prop="houseAssessTotalPrice" label="评估金额" sortable>
				</el-table-column>
				<el-table-column prop="renovationPrice" label="装修金额" sortable>
				</el-table-column>
				<el-table-column prop="reviewStatus" label="状态" sortable>
					<template slot-scope="scope">
						...
						<div v-if="scope.row.reviewStatus === 2">
							<span style="color: rgba(255, 0, 0, 0.647058823529412);">初审拒绝</span>
						</div>
						<div v-if="scope.row.reviewStatus === 3">
							<span style="color: rgba(255, 0, 0, 0.647058823529412);">初审通过</span>
						</div>
						<div v-if="scope.row.reviewStatus === 4">
							<span style="color: rgba(255, 0, 0, 0.647058823529412);">终审拒绝</span>
						</div>
						<div v-if="scope.row.reviewStatus === 5">
							<span style="color: rgba(255, 0, 0, 0.647058823529412);">终审通过</span>
						</div>
					</template>
				</el-table-column>
				<el-table-column label="操作" width="150">
					<template slot-scope="scope">
						<div v-if="scope.row.reviewStatus === 0">
							<router-link :to="{ path: '/project/mainBody', query: { 'contents': scope.row, 'conName': collectData }}">
								<span style="color: #1890FF;cursor: pointer;">主体</span>
							</router-link>
							<router-link :to="{ path: '/project/decorate', query: { 'contents': scope.row, 'conName': collectData  }}">
...
<script>
import { getRegList, postCheckId, searchHouseholdList } from '@/api/project'
export default {
  data() {
    return {
      collectData: {},
