...
<script>
	import {
		updateRegList, searchHouseholdList
	} from '@/api/project'
	export default {
		data() {
			return {
		...
		methods: {
			getHouseList(id) {
				...
				searchHouseholdList(para).then((res) => { 
					...
					this.registerData.expropriationHousehold = res.body
					this.editForm = this.registerData.expropriationHousehold
					this.editForm.name = this.collectData.name
					this.registerData.householdWorthAssesses = res.body.householdWorthAssesses
					this.tableData = this.registerData.householdWorthAssesses
				})
			},
			// 获取表格选中时的数据
